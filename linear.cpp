...
#define center_limit 5

vector<vector<float>> read_embeddings(const string& filepath) {
...
vector<float> ReadFloatsFromLine(const string& filename, int lineNumber) {
    vector<float> result;
...
float hi_distance(const vector<float>& a, const vector<float>& b, int start, int end){ ////Histogram Intersectionでは値が大きいほうが似ているとなってしまうので1から引く
    float dist = 0.0;
...
float l2_distance(const vector<float>& a, const vector<float>& b, int start, int end){       //L2距離
    float dist = 0.0;
    for (int i = start; i <= end; i++){
...
int updateNN(priority_queue<Neighbor_v>& B, int u2, float l, int k){   //更新したいpriority_queue、近傍の近傍、近傍の近傍との距離、近傍の数
...
float Select_kth_ele(const priority_queue<Neighbor_v>& B, int k){
...
vector<vector<int>> Read_graph2(float& normalized, const string& filename) {
    vector<vector<int>> result;
    ifstream file(filename);
...
float calculate_alldist(const vector<vector<float>>& data, const vector<float>& weights, vector<vector<int>>& lange, vector<float> query, int num, float max1, float max2){
    float dist1 = hi_distance(query,data[num],lange[0][0],lange[0][1]);
    float dist2 = hi_distance(query,data[num],lange[1][0],lange[1][1]);
    float dist3 = l2_distance(query,data[num],lange[2][0],lange[2][1]) / max1;
    float dist4 = l2_distance(query,data[num],lange[3][0],lange[3][1]) / max2;
    float alldist = dist1 * weights[0] + dist2 * weights[1] + dist3 * weights[2] + dist4 * weights[3];

    return alldist;
}


vector<int> select_start_node(const vector<vector<float>>& data, const  vector<vector<density>>& centers, const vector<float>& weights, const vector<vector<int>>& lange, const vector<float>& query, const vector<int>& metrics){
...
int main(){
    float graph3_max = 0.0;
    float graph4_max = 0.0;


    //数値設定
    vector<float> query = ReadFloatsFromLine("alldata.asc", 150);
    vector<float> weights = {0.2,0.2,0.3,0.3};
    vector<vector<int>> lange = {{0, 31}, {32, 63}, {64, 72}, {73, 88}};
    vector<int> metrics = {1,1,0,0}; //0はL2距離,1はHI
    vector<float> normalized = {1.0, 1.0, graph3_max, graph4_max};


    //グラフ読み込み
    vector<vector<float>> embeddings = read_embeddings("alldata.asc");
    ...
    vector<vector<density>> centers(4);
    ...
    //探索１
    vector<int> start_node = select_start_node(embeddings, centers, weights, lange, query, metrics); //初期探索点、グラフ探索順
    ...
    for(int metric=1; metric < start_node.size(); metric++){ //探索をグラフの個数回繰り返す
        if(Candidate.empty()){
            ...
            start_struct.dist = calculate_alldist(embeddings, weights, lange, query, start_node[0] , graph3_max, graph4_max);
        ...
        while(!Candidate_copy.empty()){
            ...
            for (int i=0; i < graph1[0].size(); i++){
                if(graph[start_node[metric]][q_node.num][i]==1 && visit_check[i]!=1){ //q_node.numとエッジがつながっているかつ探索されていない
                    visit_check[i] = 1;
                    if(Candidate.size() >= parameter_k+1){
                        tau = epsilon * Select_kth_ele(Candidate, parameter_k+1);
                    }
                    float dist = calculate_alldist(embeddings, weights, lange, query, i, graph3_max, graph4_max);
                    if(dist < tau){
    ...
    cout << "クエリ" << endl;
    int i=0;
    while(i < parameter_k+1){
        cout << "num: " << Candidate.top().num << ", dist: " << Candidate.top().dist << endl;
        Candidate.pop();
        i++;
    }
